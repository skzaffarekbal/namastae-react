...
## Ques): When we use Higher-Order Components (HOCs)?
...
**Ans.** Lifting state up in React refers to the practice of moving the state
...
class ParentComponent extends React.Component {
  ...
  const definition = [
    {
      ...
      description: "A comprehensive guide designed for beginners to master core programming concepts, DOM manipulation, and modern ES6+ features from scratch."
    },
    {
      title: "Advanced CSS Layouts",
      description: "Deep dive into CSS Grid, Flexbox, responsive design patterns, and subgrid features to build complex, fluid user interfaces."
    },
    {
      title: "Database Optimization",
      description: "Learn advanced indexing strategies, query execution plan analysis, and caching mechanisms to scale relational databases efficiently."
    },
    {
      title: "Securing Web Applications",
      description: "An essential overview of preventing common vulnerabilities like Cross-Site Scripting, SQL Injection, and broken authentication workflows."
    },
    {
      title: "Cloud Architecture Basics",
      description: "Understand the fundamentals of microservices, serverless deployments, managed load balancers, and global content delivery networks."
    }
  ]
  ...
  render() {
    return (
      <div>
        <p>Parent Component Active Index: {this.state.activeIndex}</p>
        {definition?.map((item, index) => {
          return (
            <ChildComponent
              activeIndex={this.state.activeIndex}
              setActiveIndex={() => this.setActiveIndex(index)}
              item={item}
...
// Child component
function ChildComponent({ activeIndex, setActiveIndex, item, show }) {
  return (
    <div>
      <p>Child Count Active Index: {activeIndex}</p>
      <p>{show.title}</p>
      <button onClick={setActiveIndex}>Expand</button>
      {show ? <p>{show.description}</p> : ''}
    </div>
  );
}
```
